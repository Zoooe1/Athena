    ...
    int il, int iu, int jl, int ju, int kl, int ku, int ngh)
{
    ...
    // which velocity component controls inflow for this face?
    const int vel =
        (face == BoundaryFace::inner_x1 || face == BoundaryFace::outer_x1) ? IVX : (face == BoundaryFace::inner_x2 || face == BoundaryFace::outer_x2) ? IVY
                                                                                                                                                      : IVZ;

    // sign convention: "inflow" means pointing *into* the domain
    // inner face inflow is +v, outer face inflow is -v
    const Real inflow_sign =
        (face == BoundaryFace::inner_x1 || face == BoundaryFace::inner_x2 || face == BoundaryFace::inner_x3)
            ? +1.0
            : -1.0;

    // Fill ghost zones by copying interior (zero-gradient), then clamp inflow velocity to 0
    // Do this for all primitive vars you use (at least density + velocities)
    for (int k = kl; k <= ku; ++k)
    {
        for (int j = jl; j <= ju; ++j)
        {
            for (int i = il - ngh; i <= iu + ngh; ++i)
            {
                ...
                // figure out which index is ghost for the specific face
                if (face == BoundaryFace::inner_x1 && i < il)
                {
                    is_ghost = true;
                    ii = il;
                }
                if (face == BoundaryFace::outer_x1 && i > iu)
                {
                    is_ghost = true;
                    ii = iu;
                }
                if (face == BoundaryFace::inner_x2 && j < jl)
                {
                    is_ghost = true;
                    jj = jl;
                }
                if (face == BoundaryFace::outer_x2 && j > ju)
                {
                    is_ghost = true;
                    jj = ju;
                }
                if (face == BoundaryFace::inner_x3 && k < kl)
                {
                    is_ghost = true;
                    kk = kl;
                }
                if (face == BoundaryFace::outer_x3 && k > ku)
                {
                    is_ghost = true;
                    kk = ku;
                }
                ...
                // Copy primitives from nearest interior cell
