...
namespace
{
    struct SinkParams
    {
        ...
        Real r_noz_in;  // radius of nozzle
        Real r_noz_out;
    ...
    inline bool in_hollow_cone_plus(const Real x, const Real y, const Real z)
    {
        ...
        if (h > S.L_noz || h < 0.0)
            return false;
        const Real r_max = h * (S.r_noz_out / S.L_noz);
        const Real r_min = h * (S.r_noz_in / S.L_noz);
        return (r_perp <= r_max && r_perp >= r_min);
    }
    inline bool in_hollow_cone_minus(const Real x, const Real y, const Real z)
    {
        const Real r_perp = std::sqrt((x - S.x0) * (x - S.x0) + (y - S.y0) * (y - S.y0));
        const Real zapex = S.z0 - S.r_sink;
...
{ // a pointer parameter that allow us to read the data from athinput.sinkjet
    ...
    if (S.jet_switch)
    {
        S.r_noz_in = pin->GetReal("problem", "r_noz_in");
        S.r_noz_out = pin->GetReal("problem", "r_noz_out");
        ...
        S.A_noz = M_PI * (S.r_noz_out * S.r_noz_out - S.r_noz_in * S.r_noz_in); // area per lobe
    ...
    for (int k = pmb->ks; k <= pmb->ke; ++k)
    {
        ...
        for (int j = pmb->js; j <= pmb->je; ++j)
        {
            ...
            for (int i = pmb->is; i <= pmb->ie; ++i)
            {
                ...
                {
                    in_noz = in_hollow_cone_plus(xc, yc, zc) || in_hollow_cone_minus(xc, yc, zc); // both nozzle
                }
                if (in_noz)
    ...
    if (S.jet_switch)
    {
        ...
        for (int k = pmb->ks; k <= pmb->ke; ++k)
        {
            ...
            for (int j = pmb->js; j <= pmb->je; ++j)
            {
                ...
                {
                    const Real xc = x1v(i);
                    if (in_hollow_cone_plus(xc, yc, zc) || in_hollow_cone_minus(xc, yc, zc))
                    {
        ...
        // if (Globals::my_rank == 0)
        // {
        //     std::cout << "r_noz_in=" << S.r_noz_in
        //               << " r_noz_out=" << S.r_noz_out
        //               << " L_noz=" << S.L_noz << "\n";
        // }
        ...
        if (V_nozzle_total > 0.0 && M_need > 0.0 && M_injected > 0.0)
        {
            ...
            for (int k = pmb->ks; k <= pmb->ke; ++k)
            {
                ...
                for (int j = pmb->js; j <= pmb->je; ++j)
                {
                    ...
                    {
                        const Real xc = x1v(i);
                        const bool plus = in_hollow_cone_plus(xc, yc, zc);              // in the upper lobe
                        const bool minus = (!plus) && in_hollow_cone_minus(xc, yc, zc); // in the downer lobe
                        if (!(plus || minus))
