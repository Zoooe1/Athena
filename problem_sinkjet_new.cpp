...
namespace
{
    struct SinkParams
    {
        ...
        // for data logging
        std::FILE *fp = nullptr; // a pointer that will fill a CSV file only when you open it, std:: means it's a standard C++ library namespace
...
{ // a pointer parameter that allow us to read the data from athinput.sinkjet
    ...
    if (S.fp)
        std::fprintf(S.fp, "# time,Msink,Mgas,SFE_M0,SFE_inst\n"); // if able to open the file write the headers
                   ...
                   AthenaArray<Real> &cons_scalar)
{
    ...
    for (int k = pmb->ks; k <= pmb->ke; ++k)
    {
        for (int j = pmb->js; j <= pmb->je; ++j)
        {
            for (int i = pmb->is; i <= pmb->ie; ++i)
            {
                ...
                Mgas_before += cons(IDN, k, j, i) * V;
            }
        }
    }
    Real Msink_before = S.Msink;
    Real dMsink_accrete = 0.0;
    ...
    if (S.jet_switch)
    {
        ...
        if (V_nozzle_total > 0.0)
        {
            ...
            for (int k = pmb->ks; k <= pmb->ke; ++k)
            {
                ...
                for (int j = pmb->js; j <= pmb->je; ++j)
                {
                    ...
                    for (int i = pmb->is; i <= pmb->ie; ++i)
                    {
                        ...
                        const Real dM = dM_total * (Vcell / V_nozzle_total); // find the inject jet mass distributed per cell
            ...
            const Real M = std::max(S.Msink, 0.0); // mass that sink has
            const Real dM_inject = std::min(dM_total, M);
            S.Msink -= dM_inject;
        }
    }
    ...
    for (int k = pmb->ks; k <= pmb->ke; ++k)
    {
        for (int j = pmb->js; j <= pmb->je; ++j)
        {
            ...
            {
                const Real V = pmb->pcoord->GetCellVolume(k, j, i);
                Mgas_after += cons(IDN, k, j, i) * V;
            }
        }
    }

    Real Mtot_before = Msink_before + Mgas_before;
    Real Mtot_after = S.Msink + Mgas_after;
    if (S.M0 < 0.0)
    {
        S.M0 = Mtot_before;
    }
    ...
    // DEBUG statement
    std::printf("MASS_DEBUG t=%e Mb=%e Ma=%e Msinkb=%e Msinka=%e M0=%e Mtot_after=%e\n",
                time,
                Mgas_before, Mgas_after,
                Msink_before, S.Msink,
                S.M0, Mtot_after);
    ...
    if (time > last_t_written)
    {
        if (S.M0 < 0.0)
            S.M0 = Mtot_before;
        // Two SFE definitions
        const Real sfe_M0 = S.Msink / (S.M0 > 0 ? S.M0 : 1.0);                 // What fraction of the original gas has become stars by       time t
        const Real sfe_inst = S.Msink / std::max(S.Msink + Mgas_after, 1e-60); // SFE_intantaneous = Msink / (Msink + gas at the point now)
